...
public class EventRenderGame extends Gui
{
    ...
    @SubscribeEvent public void preRenderGameOverlay (RenderGameOverlayEvent.Pre event)
    {
        ...
        for (int k4 = 0; k4 < l3; ++k4)
        {
            ...
            if (k4 < list.size())
            {
                ...
                if (indicatorRecIndex != IndexReference.ICON_DISABLE_INDEX)
                    drawTexturedModalRect(j2 - (flag ? 9 : 0) + userWidth - INDICATOR_TOTAL + INDICATOR_PADDING, k2, indicatorRecIndex * 8,
                            (int) Math.floor(indicatorRecIndex / 32) * 8 + IndexReference.RES_SSD, INDICATOR_WIDTH, INDICATOR_WIDTH);
                drawTexturedModalRect(j2 - (flag ? 9 : 0) + userWidth - INDICATOR_TOTAL + INDICATOR_PADDING * 2 + INDICATOR_WIDTH, k2, indicatorStrIndex * 8,
                        (int) Math.floor(indicatorStrIndex / 32) * 8 + IndexReference.RES_SSD, INDICATOR_WIDTH, INDICATOR_WIDTH);
    ...
    @SubscribeEvent public void renderGameOverlay (RenderGameOverlayEvent.Post event)
    {
        Minecraft mc = Minecraft.getMinecraft();
        ResourceLocation indicators = Minecraft.isFancyGraphicsEnabled() ? indicatorsHigh : indicatorsLow;
